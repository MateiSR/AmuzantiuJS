...
const getQueuePage = function(queueSliced, pageNum) {
    // pageNum = index + 1
    let songLength;
    var formattedQueue = [];
    let c, c2, ret;
...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (parsedPageNum != NaN) {
            if (parsedPageNum <= queueSliced.length && parsedPageNum >= 1) return await message.channel.send(getQueuePage(queueSliced, parsedPageNum));
        }
    }
}